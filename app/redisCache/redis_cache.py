...
def set_cache(key: str, value: str, ttl: int = None):
    ...
    try:
        if ttl:
            ...
            logging.info(
                "Key '%s' set with value '%s' and will expire in %d seconds.",
                key,
                value,
                ttl,
            )
...
def set_cache_with_midnight_expiration(key: str, value: str):
    ...
    try:
        ...
        logging.info(
            "Key '%s' set with value '%s' and will expire in %d seconds (by midnight).",
            key,
            value,
            ttl,
        )
...
def reset_cache_at_midnight(key: str):
    ...
    try:
        ...
        logging.info(
            "Key '%s' reset with an empty value and will expire in %d seconds (by midnight).",
            key,
            ttl,
        )
    except Exception as e:
