...
from redisCache.redis_cache import get_cache, set_cache_with_midnight_expiration, reset_cache
...
async def get_daily_prompt(db_connection=Depends(get_db_connection)):
    try:
        if get_cache("daily_prompt"):
            ...
            set_cache_with_midnight_expiration("daily_prompt", json.dumps({"prompt": result[0], "emotion": result[1]}))
...
async def create_collective_prompt(collective_prompt: collective_prompt, request:Request, db_connection=Depends(get_db_connection)): 
  ...
  if client_ip.startswith(('192.', '10.', '172.')) or client_ip in ['127.0.0.1', '0.0.0.0']:
        client_ip = "me"
  ...
  try:
    ...
    query = """
            ...
            """
    cursor.execute(query, (data["user_text"], data["emotion"], data["ip_address"], data["prompt"], data["latitude"], data["longitude"]))
